 ...
 * @since 10.12.14
 */
public class ChangeEntryDaoTest extends AbstractDaoTest {
  ...
  public void shouldCreateChangeIdAuthorIndexIfNotFound() throws MongobeeConfigurationException {
    ...
    ChangeEntryDao dao =
        new ChangeEntryDao(
            ...
            WAIT_FOR_LOCK,
            CHANGE_LOG_LOCK_WAIT_TIME,
            CHANGE_LOG_LOCK_POLL_RATE,
            THROW_EXCEPTION_IF_CANNOT_OBTAIN_LOCK);

    ChangeEntryIndexDao indexDaoMock = mock(ChangeEntryIndexDao.class);
    when(indexDaoMock.findRequiredChangeAndAuthorIndex(mongoDatabase)).thenReturn(null);
  ...
  public void shouldNotCreateChangeIdAuthorIndexIfFound() throws MongobeeConfigurationException {
    ...
    ChangeEntryDao dao =
        new ChangeEntryDao(
            ...
            THROW_EXCEPTION_IF_CANNOT_OBTAIN_LOCK);
    ChangeEntryIndexDao indexDaoMock = mock(ChangeEntryIndexDao.class);
    when(indexDaoMock.findRequiredChangeAndAuthorIndex(mongoDatabase)).thenReturn(new Document());
    ...
    verify(indexDaoMock, times(0))
        .createRequiredUniqueIndex(mongoDatabase.getCollection(CHANGELOG_COLLECTION_NAME));
    // and not
    verify(indexDaoMock, times(0))
        .dropIndex(mongoDatabase.getCollection(CHANGELOG_COLLECTION_NAME), new Document());
  }

  @Test
  public void shouldRecreateChangeIdAuthorIndexIfFoundNotUnique()
    ...
    ChangeEntryDao dao =
        new ChangeEntryDao(
            ...
            THROW_EXCEPTION_IF_CANNOT_OBTAIN_LOCK);
    ChangeEntryIndexDao indexDaoMock = mock(ChangeEntryIndexDao.class);
    when(indexDaoMock.findRequiredChangeAndAuthorIndex(any(MongoDatabase.class)))
  ...
  public void shouldInitiateLock() throws MongobeeConfigurationException {
    ...
    // then
    verify(lockDao).intitializeLock(any(MongoDatabase.class));
  }
  ...
  public void shouldCheckLockHeldFromFromLockDao() throws Exception {
    ...
    // when
    when(lockDao.isLockHeld(any(MongoDatabase.class))).thenReturn(true);
